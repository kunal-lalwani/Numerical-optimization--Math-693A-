...
pk = - hessF_value \ gradF_value; %search direction
...
while(iterator <= 100)
    ...
    pk = - hessF_value \ gradF_value; %recalculating search direction
...
%         xk = x_next;
    ...
    %error limit |xk+1 - xk|
    if_cond = abs(abs(func_next) - abs(func_prev));
    if( if_cond < 10^(-6))
        break;
    end
%     figure(1.1)
